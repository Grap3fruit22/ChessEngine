...
def AlgoDuel(weightsA,weightsB,Quantity):
    ...
    for num in range(0,Quantity):
        ...
        if board.result() == '0-1':
            weightsA = (9/10)*np.asarray(weightsA) + (1/10)*np.asarray(weightsB)
        
    return [results.count('1-0'),results.count('0-1'),weightsA]
...
def RunBasicGeneticExperiement(InitialWeight,Generations,Playouts):
    ...
    while counter<Generations:
        ...
        [W,L,blank] = AlgoDuel(WeightA,WeightB,Playouts)
        Tot = W+L
        if (Tot!=0):
...
def RunAdvancedGeneticExperiement(InitialWeight,Generations,Playouts):
    ...
    while counter < Generations:
        ...
        for mask in MaskList:
            ...
            [W,L,WeightA] = AlgoDuel(WeightA,WeightB,Playouts)
...
RunAdvancedGeneticExperiement([1,0.955,1.05,1.5,8.5],14,5)
